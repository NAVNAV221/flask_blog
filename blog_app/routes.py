...
def home():
    ...
    return render_template('home.html', title='Home', posts=posts.items, form=form, next_url=next_url,
                           prev_url=prev_url)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Congratulations, you now registered new user!')
        return redirect(url_for('login'))
...
def follow(username):
    ...
    if user is None:
        flash('Username {} is not found'.format(username))
        return redirect(url_for('home'))
...
def unfollow(username):
    ...
    db.session.commit()
    flash('You now unfollow {}'.format(username))
    return redirect(url_for('user', username=username))
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Follow the instructions to reset your password')
        return redirect(url_for('login'))
...
@app.route('/test_home_page')
def test_home_page():
    return render_template('test_home_page.html', title='TEST_HOME_PAGE')